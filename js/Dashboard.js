            ...
            if (orphanNotes.length > 0) {
                ...
                orphanNotes.forEach(note => {
                    ...
                    noteItem.innerHTML = `
                        ...
                        <span>${note.name}</span>
                    `;
                    noteItem.onclick = () => this.loadBoard(note.id);
        ...
        folders.forEach(folder => {
            ...
            if (isExpanded) {
                ...
                if (this.isMobile()) {
                    ...
                    newNoteBtn.className = 'btn-new-note-tree';
                    newNoteBtn.style.paddingLeft = `${32 + level * 20}px`;
                    newNoteBtn.innerHTML = `<span>+ Yeni Not</span>`;
                    newNoteBtn.onclick = (e) => {
                        e.stopPropagation();
                        this.currentBoardId = null; // Reset current board ID for new note logic
                        this.switchView(folder.id);
                        this.createNewBoard();
                    };
                    childContainer.appendChild(newNoteBtn);
                }
