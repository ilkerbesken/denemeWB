class PageManager {
    ...
    getPageHeight() {
        if (this.app.canvasSettings) {
            ...
            if (size === 'full') {
                return this.app.canvas.clientWidth; // Dikey dökümanda Full'u viewport genişliği kadar alalım
            }
    ...
    getPageWidth() {
        if (this.app.canvasSettings) {
            const size = this.app.canvasSettings.settings.size;
            if (size === 'full') {
                return this.app.canvas.clientWidth;
            }
            const dimensions = this.app.canvasSettings.sizes[size];
    ...
    switchPage(index, shouldScroll = true) {
        if (index < 0 || index >= this.pages.length) return;

        // Mevcut durumu kaydet (Geçiş yapmadan önce)
        this.saveCurrentPageState();
