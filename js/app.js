class WhiteboardApp {
                ...
                if (selectTool.selectedObjects.length === 1) {
                    ...
                    if (obj.type === 'table') {
                        ...
                        if (action === 'addRowAbove') {
                            ...
                            obj.rows++;
                            obj.rowHeights.unshift(cellHeight);
                            obj.data.unshift(newRowData);
                            obj.height += cellHeight;
                        } else if (action === 'addRowBelow') {
                            // Insert at BOTTOM
                            const newRowData = Array(obj.cols).fill("");
                            obj.rows++;
                            obj.rowHeights.push(cellHeight);
                            obj.data.push(newRowData);
                            obj.height += cellHeight;
                        } else if (action === 'addColLeft') {
                            // Insert at LEFT
                            obj.cols++;
                            obj.colWidths.unshift(cellWidth);
                            obj.width += cellWidth;
                            obj.data.forEach(row => row.unshift(""));
                        } else if (action === 'addColRight') {
                            // Insert at RIGHT
                            obj.cols++;
                            obj.colWidths.push(cellWidth);
                            obj.width += cellWidth;
                            obj.data.forEach(row => row.push(""));
                        } else if (action === 'deleteRow') {
                            if (obj.rows > 1) {
                                obj.rows--;
                                // For now delete last one as simple behavior
                                const h = obj.rowHeights.pop();
                                obj.height -= h;
                                obj.data.pop();
                            }
                            ...
                            if (obj.cols > 1) {
                                obj.cols--;
                                // For now delete last one as simple behavior
                                const w = obj.colWidths.pop();
                                obj.width -= w;
                                obj.data.forEach(row => row.pop());
                            }
                        }
                    }
                }
    ...
    redrawOffscreen() {
        ...
        this.offscreenCtx.fillRect(0, 0, logicalW, logicalH);
        ...
        const worldBounds = {
            ...
            w: logicalW / this.zoomManager.zoom,
            h: logicalH / this.zoomManager.zoom
        };
    ...
    render() {
        ...
        if (this.offscreenCanvas.width > 0 && this.offscreenCanvas.height > 0) {
            ...
            this.ctx.drawImage(this.offscreenCanvas, 0, 0, LOGICAL_WIDTH, LOGICAL_HEIGHT);
